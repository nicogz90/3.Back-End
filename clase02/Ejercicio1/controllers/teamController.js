...
const destroy = async (req, res) => {
  if (db.teams.hasOwnProperty(req.params.id)) {
    ...
    delete db.teams[req.params.id]; // si pongo 'delete team' borra la constante que se definió en la linea anterior, pero no el team que quiero quitar del listado
    res.json(team);
  } else {
    res.status(404).json({ error: "Equipo no encontrado" });
  }
